...
class UCaps3D(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        # self.reconstruction_loss = nn.L1Loss(reduction="none")  # SSIM()
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument("--cls_loss", type=str, default="CE")  # CE, Dice, DiceCE
    ...
    #     self.save_csv(self.CEloss, "/mnt/Data/Cryo-ET/3D-UCaps/")
    ...
    def training_step(self, batch, batch_idx):
        ...
        # self.log("margin_loss", cls_loss[0], on_step=True, on_epoch=False, sync_dist=False)
        # self.log(f"{self.cls_loss}_loss", cls_loss[1], on_step=True, on_epoch=False, sync_dist=False)
        # self.log("reconstruction_loss", rec_loss, on_step=True, on_epoch=False, sync_dist=False)

        return loss

    # def save_csv(self, data, output_path, flag="Train", name="Probabilities"):
    ...
    def losses(self, volumes, labels, norm, pred, reconstructions):
        ...
        # rec_loss = 0.84 * (1 - ssim(labels, pred,size_average=True)[0]) + (1 - 0.84) * (torch.sum(torch.abs(labels - pred) * 11) / (volumes.shape[2]*1))
    ...
    def _build_decoder(self):
        ...
        if self.connection == "skip":
            ...
            # print(self.encoder_output_atoms[-1])
            self.decoder_in_channels = [self.out_channels * self.encoder_output_atoms[-1], 192, 64, 128]
            self.decoder_out_channels = [128, 64, 64, self.out_channels]
